...
exports.createUser = async (req, res) => {
  try {
    ...
    res.status(201).json({
      message:
        "Account created successfully... Kindly check your email for verification",
      user,
    });
  } catch (err) {
    res.status(500).json({
...
exports.changeProfilePics = async (req, res) => {
  try {
    ...
    await userModel.findByIdAndUpdate(
      ID,
      { profileImage: newImage.secure_url },
      { new: true }
    );
...
exports.updateUser = async (req, res) => {
  try {
    ...
    // eddit the details
    const updated = await userModel.findByIdAndUpdate(ID, editOnly, {
      new: true,
    });
    if (!updated) {
      return res.status(400).json({
